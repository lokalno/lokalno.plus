...
import {
  ...
  SELLER_CANCEL_REASON_LABELS,
  type SellerCancelReason,
} from "@/lib/order-cancel";

type OrderSellerCancelDialogProps = {
  open: boolean;
  loading: boolean;
  error: string;
  onClose: () => void;
  onConfirm: (reason: SellerCancelReason, note: string) => void;
};

export default function OrderSellerCancelDialog({
  open,
  loading,
  error,
  onClose,
  onConfirm,
}: OrderSellerCancelDialogProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl border bg-white p-5 shadow-xl">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Оберіть причину — покупець отримає сповіщення.
        </p>
        ...
        <div className="mt-4 space-y-2">
          ...
          <select
            value={reason}
            onChange={(e) => setReason(e.target.value as SellerCancelReason | "")}
            className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm"
          >
            <option value="">Оберіть причину</option>
            {SELLER_CANCEL_REASONS.map((item) => (
              <option key={item} value={item}>
                {SELLER_CANCEL_REASON_LABELS[item]}
              </option>
            ))}
          </select>
        </div>
