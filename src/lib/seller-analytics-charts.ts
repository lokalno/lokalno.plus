...
export function buildSellerAnalyticsSeries(
  ...
  if (period === "month") {
    ...
    for (const order of inPeriod) {
      ...
      addOrderToPoint(points[date.getDate() - 1], order);
  ...
  if (period === "year") {
    ...
    const points = Array.from({ length: 12 }, (_, index) => {
      ...
      return {
        ...
        ...initPoint(formatMonthLabel(date)),
    ...
    for (const order of inPeriod) {
      ...
      addOrderToPoint(points[date.getMonth()], order);
  ...
  const points = Array.from({ length: endYear - startYear + 1 }, (_, index) => {
    ...
    return { ...initPoint(String(year)), year };
  ...
  for (const order of inPeriod) {
    ...
    const point = points.find((item) => item.year === date.getFullYear());
